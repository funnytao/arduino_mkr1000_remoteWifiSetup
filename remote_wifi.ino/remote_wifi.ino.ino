...
void loop() {
  ...
  //            Serial.println("11111 ssid="+newSSID);
  ...
  //              Serial.println("11111 password="+newPASS);
              }
            } 
          }
        }
    ...
    if (newSSID.length() > 0) {
      char* wifissid = strdup(newSSID.c_str());
  //    for (int i = 0; i < newSSID.length(); i++) {
  //      wifissid[i] = newSSID.charAt(i);
  //    }
      char* wifipass = strdup(newPASS.c_str());
  //    for (int i = 0; i < newPASS.length(); i++) {
  //      wifipass[i] = newPASS.charAt(i);
  //    }
  //      Serial.println("The ssid = "+wifissid);
...
void connectToWiFi(char* ssid_wifi, char* pass_wifi) {
  ...
  while ( status != WL_CONNECTED) {
    ...
    if (newPASS.length() > 0) {
      status = WiFi.begin(ssid_wifi, pass_wifi);
    }
    else {
      status = WiFi.begin(ssid_wifi);
    }
    // wait 10 seconds for connection:
    delay(10000);
    connected++;
